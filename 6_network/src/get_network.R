...
compute_up_down_stream <- function(reaches_bounded) {
  #----------------
  #~26 minutes for national single-threaded
  get_reach_direction <- function(nat_seg_ids, reaches_bounded) {
    `%>%` <- magrittr::`%>%`
    seg_ids_df <- dplyr::filter(reaches_bounded, seg_id_nat %in% nat_seg_ids)
      ...
      dplyr::mutate(
        which_end_up = sapply(seg_id, function(segid) {
          ...
          if(!is.na(reach$to_seg)) { # non-river-mouths
            ...
            which.max(sf::st_distance(reach$end_points[[1]], sf::st_geometry(to_reach)))
          } else { # river mouths
            # upstream endpoint is the one closest to a previous reach upstream
            from_reach <- dplyr::filter(reaches_bounded, to_seg == reach$seg_id)[1,] # only need one
            which.min(sf::st_distance(reach$end_points[[1]], sf::st_geometry(from_reach)))
  ...
  nat_seg_ids_split <- clusterSplit(cl, reaches_bounded$seg_id_nat)
  #clusterExport(cl, varlist = "reaches_bounded", envir = environment())
  cluster_results <- parLapply(cl = cl, X = nat_seg_ids_split, 
                               fun = get_reach_direction, reaches_bounded)
 
  return(reaches_bounded_direction)
}
