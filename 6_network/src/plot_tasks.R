make_site_reach_plots <- function(outind, reaches_to_check, category, ...) {
  ...
  task_names <- as.character(unique(reaches_to_check_category$seg_id))
  ...
  create_task_makefile(
    ...
    finalize_funs = 'summarize_plots',
    final_targets = 'reach_plots_inventory',
    as_promises = TRUE
  )
  
  loop_tasks(task_plan = task_plan, task_makefile = task_makefile_name,
            num_tries = 1)
  plot_inventory <- remake::fetch('reach_plots_inventory_promise', 
                                  remake_file = task_makefile_name) 
